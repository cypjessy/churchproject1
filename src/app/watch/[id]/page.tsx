...
export default function WatchVideoPage() {
    ...
    return (
      <div className="app-container">
        ...
        <div className="sk-topbar">
          <div className="sk sk-back"></div>
          <div className="sk sk-top-title"></div>
        </div>

        {/* Player skeleton with pulsing play button */}
        <div className="sk-player">
          <div className="sk-play-pulse"></div>
        </div>
        ...
        <div className="sk-actions">
          <div className="sk sk-action"></div>
          <div className="sk sk-action"></div>
          <div className="sk sk-action"></div>
          <div className="sk sk-action"></div>
        </div>
        ...
        <div className="sk-channel">
          <div className="sk sk-avatar"></div>
          <div>
            <div className="sk sk-channel-name"></div>
            <div className="sk sk-channel-sub"></div>
          </div>
        </div>
        ...
        <div className="sk-related">
          <div className="sk sk-related-title"></div>
          {[1,2,3].map(i => (
            <div className="sk-related-item" key={i}>
              <div className="sk sk-related-thumb"></div>
              <div className="sk-related-info">
                <div className="sk sk-related-name"></div>
                <div className="sk sk-related-name"></div>
                <div className="sk sk-related-meta"></div>
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="app-container">
        <div className="w-notfound">
          ...
          <button className="w-back-btn" onClick={() => router.back()}>Go Back</button>
        </div>
  ...
  const durationSec = parseISOToSeconds(video.duration);
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif}
        ...
        .w-notfound{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;gap:12px;color:var(--text-tertiary);font-size:14px}
        .w-notfound i{font-size:48px;color:var(--text-tertiary)}
        .w-notfound h3{font-size:18px;color:var(--text-primary);margin-top:4px}
        .w-back-btn{padding:10px 24px;border-radius:12px;background:var(--surface);border:1px solid var(--border);color:var(--text-primary);font-size:14px;font-weight:600;cursor:pointer;margin-top:8px}
        ...
        .w-player-thumb{width:100%;height:100%;object-fit:cover}
        .w-player-overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.3);cursor:pointer;transition:opacity 0.3s}
        .w-player-overlay:hover{opacity:0.9}
        .w-play-btn{width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,var(--gradient-start),var(--gradient-end));display:flex;align-items:center;justify-content:center;font-size:26px;color:#fff;box-shadow:0 6px 40px rgba(232,168,56,0.4)}
        .w-progress-badge{position:absolute;bottom:12px;right:12px;padding:4px 10px;border-radius:6px;background:rgba(0,0,0,0.75);color:#fff;font-size:12px;font-weight:700}
        .w-yt-embed-trigger{position:absolute;bottom:12px;left:12px;padding:6px 14px;border-radius:8px;background:rgba(255,0,0,0.85);color:#fff;font-size:11px;font-weight:700;display:flex;align-items:center;gap:6px;cursor:pointer;border:none}
        .w-yt-embed-trigger:active{opacity:0.8}
        ...
        .w-series-tag:active{background:rgba(232,168,56,0.12)}
        ...
        .w-desc-toggle:active{color:var(--text-secondary)}
        ...
        .w-related-title i{color:var(--primary);font-size:14px}
        ...
        .w-related-item:active .w-related-thumb img{transform:scale(1.05)}
        ...
        .w-empty-related{padding:20px;text-align:center;color:var(--text-tertiary);font-size:13px}
      `}</style>
      ...
      <div className="app-container">
        ...
        <div className="w-topbar">
          <button className="w-back" onClick={() => router.back()}>
        ...
        <div className="w-scroll">
          ...
          <div className="w-info">
            ...
            <div className="w-meta-row">
              ...
              <span
                className="w-category-tag"
                style={{
                  background: `${CATEGORY_COLORS[video.category] || "#E8A838"}15`,
                  color: CATEGORY_COLORS[video.category] || "var(--primary)",
                }}
              >
                <span className="w-cat-dot" style={{ background: CATEGORY_COLORS[video.category] || "var(--primary)" }}></span>
                {video.category}
              </span>
            </div>
            ...
            <div className="w-actions">
              <button className={`w-action-btn ${liked ? "active" : ""}`} onClick={handleLike}>
                <i className={`fas fa-${liked ? "heart" : "heart"}`}></i>
                {liked ? "Liked" : "Like"}
              </button>
              <button className="w-action-btn" onClick={handleShare}>
                <i className="fas fa-share-nodes"></i> Share
              </button>
              <button className={`w-action-btn ${saved ? "active" : ""}`} onClick={handleSave}>
                <i className={`fas fa-${saved ? "bookmark" : "bookmark"}`}></i>
                {saved ? "Saved" : "Save"}
              </button>
              <button className="w-action-btn primary" onClick={handleWatchOnYT}>
            ...
            <div className="w-channel-bar">
              <div className="w-channel-avatar">
              ...
              {seriesName && (
                <div className="w-series-tag" onClick={() => { router.push("/watch"); }}>
                  <i className="fas fa-list"></i> {seriesName}
                </div>
              )}
            </div>
            ...
            {video.description && (
              <>
                <div className={`w-desc ${!showDesc ? "collapsed" : ""}`}>
                  {video.description}
                </div>
                {video.description.length > 150 && (
                  <button className="w-desc-toggle" onClick={() => setShowDesc(!showDesc)}>
                    {showDesc ? "Show less" : "Show more"} <i className={`fas fa-chevron-${showDesc ? "up" : "down"}`} style={{ fontSize: 10, marginLeft: 4 }}></i>
                  </button>
                )}
              </>
            )}
          ...
          <div className="w-related">
            <div className="w-related-title">
              <i className="fas fa-list"></i> Related Videos
            </div>
            {relatedVideos.length === 0 ? (
              <div className="w-empty-related">No related videos found</div>
            ) : (
              <div className="w-related-list">
                {relatedVideos.slice(0, 10).map((rv) => (
                  <div
                    className="w-related-item"
                    key={rv.youtubeId}
                    onClick={() => router.push(`/watch/${rv.youtubeId}`)}
                  >
                    <div className="w-related-thumb">
                      <img src={rv.thumbnail} alt={rv.title} loading="lazy" />
                      <span className="w-related-duration">{rv.duration}</span>
                    </div>
                    <div className="w-related-info">
                      <div className="w-related-name">{rv.title}</div>
                      <div className="w-related-meta">
                        <span>{formatCount(rv.views)} views</span>
                        <span>·</span>
                        <span>{formatDate(rv.publishedAt).split(",")[0]}</span>
                      </div>
                      <div className="w-related-meta" style={{ marginTop: 2 }}>
                        <span className="w-cat-dot" style={{ width: 5, height: 5, background: CATEGORY_COLORS[rv.category] || "var(--primary)" }}></span>
                        <span style={{ textTransform: "capitalize" }}>{rv.category}</span>
                      </div>
                    </div>
                  </div>
                ))}
